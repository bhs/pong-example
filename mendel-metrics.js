 ...
 * 'game-customization-options' hop (in-canvas menu overlay variation).
 *
 ...
 *
 * Configuration is read explicitly from the environment (see README of the
 * task): MENDEL_METRICS_ENDPOINT, MENDEL_METRICS_TOKEN, MENDEL_EXPERIMENT_ID,
 * and (optional) MENDEL_EXPERIMENT_BUCKETS. When any of the first three are
 ...
 *   customization_changed / page_view  (primary,   higher is better)
 *   game_completed        / page_view  (guardrail, higher is better)
 */
...
// 'game-customization-options' declares (and increments) these same
// counters, whatever UI it uses to let the player change settings.
const DECLARED_METRICS = [
  {
    name: 'customization_changed', per: 'page_view', role: 'primary', better: 'higher',
    description: 'Times a player changes a customization option (paddle color / ball speed / difficulty) per page view',
  },
  {
    name: 'game_completed', per: 'page_view', role: 'guardrail', better: 'higher',
    description: 'Completed games (a match reaching game-over) per page view',
  },
];
...
function setupMetrics() {
  ...
  try {
    ...
    const meter = provider.getMeter('game-customization-options');
    ...
    DECLARED_METRICS.forEach((m) => {
      if (!counters[m.name]) counters[m.name] = meter.createCounter(m.name);
      if (!counters[m.per])  counters[m.per]  = meter.createCounter(m.per);
    });
 ...
 * mendel_server_errors/mendel_participants, and counts a 'page_view' event
 * for each top-level load of the game page. Stashes the attribute set used
 * for this request on res.locals.mendelAttrs so route handlers can report a
 * declared event (e.g. customization_changed) with the same attribution.
 *
...
function mendelMiddleware(req, res, next) {
  ...
  // named in MENDEL_EXPERIMENT_BUCKETS (set upstream of this code — this
  // experiment's assignment_unit is 'device', so no server-side bucket
  // computation happens here; we only read what's already present).
  ...
  // page_view — one of this hop's declared 'per' events: counted for every
  // top-level load of the game page.
  if (req.method === 'GET' && (req.path === '/' || req.path === '/index.html')) {
    incr('page_view', attrs);
  }
