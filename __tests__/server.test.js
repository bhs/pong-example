...
describe('GET /api/preferences', () => {
  ...
  test('repeat requests are served from the in-process LRU cache, not Prisma', async () => {
    const prisma = createFakePrisma();
    prisma.__preferences.set(ALICE.id, {
      id: 1, userId: ALICE.id, theme: 'dark', soundEnabled: true, paddleColor: '#123456',
      createdAt: new Date(), updatedAt: new Date(),
    });
    const findUniqueSpy = jest.spyOn(prisma.preference, 'findUnique');
    const app = createApp(prisma, { testAuth: loggedInAs(ALICE) });

    const first  = await get(app, '/api/preferences');
    const second = await get(app, '/api/preferences');

    expect(first.body.preferences.paddleColor).toBe('#123456');
    expect(second.body.preferences.paddleColor).toBe('#123456');
...
describe('PUT /api/preferences', () => {
  ...
  test('returns 400 for a paddleColor that is not a hex color', async () => {
    const app = createApp(createFakePrisma(), { testAuth: loggedInAs(ALICE) });
    const res = await put(app, '/api/preferences', { paddleColor: 'red' });
    expect(res.status).toBe(400);
  });

  test('saves ballColor, bgColor and presetName together', async () => {
    const prisma = createFakePrisma();
    const app    = createApp(prisma, { testAuth: loggedInAs(ALICE) });
    ...
    expect(res.status).toBe(200);
    expect(res.body.preferences.ballColor).toBe('#ff2ec4');
    expect(res.body.preferences.bgColor).toBe('#0d0221');
    expect(res.body.preferences.presetName).toBe('neon');
  });

  test('returns 400 for an invalid ballColor', async () => {
    const app = createApp(createFakePrisma(), { testAuth: loggedInAs(ALICE) });
    const res = await put(app, '/api/preferences', { ballColor: 'not-a-color' });
    expect(res.status).toBe(400);
  });

  test('returns 400 for a non-string, non-null presetName', async () => {
    const app = createApp(createFakePrisma(), { testAuth: loggedInAs(ALICE) });
    const res = await put(app, '/api/preferences', { presetName: 42 });
    expect(res.status).toBe(400);
  });

  test('a later GET reflects a saved presetName (write-through cache)', async () => {
    const prisma = createFakePrisma();
    const app    = createApp(prisma, { testAuth: loggedInAs(ALICE) });

    await put(app, '/api/preferences', { presetName: 'classic', paddleColor: '#ffffff' });
    const res = await get(app, '/api/preferences');

    expect(res.body.preferences.presetName).toBe('classic');
  });
});
