...
describe('DECLARED_METRICS', () => {
  test('declares exactly one primary metric', () => {
  ...
  test('includes the primary customization_changed / page_view metric', () => {
    const primary = DECLARED_METRICS.find((m) => m.role === 'primary');
    expect(primary.name).toBe('customization_changed');
    expect(primary.per).toBe('page_view');
  });
});
...
describe('incr / recordDuration (metrics disabled in test env)', () => {
  test('incr never throws for any counter name', () => {
    expect(() => incr('page_view', {})).not.toThrow();
